...
class ExpenditureView(View):
    def get(self, request):
        data = {
            "expenditures":Expenditure_Records.objects.all().order_by("expenditure")
        }
...
class IncomeView(View):
    def get(self, request):
        data = {
            "income": Income_Records.objects.all().order_by("income")
        }
...
class RecordTransfersView(View):
    def get(self, request):
        data = {
            "transfers":Transfer.objects.all().order_by("-value")[:151]
        }
...
class PredictionsView(View):
    def get(self, request):
        data = {
            "predictions": Predictions.objects.all().order_by("divergence", "-value")[:151]
        }
